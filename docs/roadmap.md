...
| **v0.2.0** | 2026-05-14 | 5 first-party LLM provider sister packages (`agentforge-anthropic`, `-openai`, `-voyage`, `-litellm`, `-ollama`); 5 new runbooks (17–21); sentence-window streaming guardrails (feat-020 v0.3 polish); native lexical paths on every shipped vector store (`feat-022 native + feat-025`); GraphRAG retrieval (feat-023); reranker contract + 4 vendor sister packages (feat-021); schema migrations framework + parameterised migrations (feat-024); production MCP + A2A runners; vendor observability backends (Langfuse / Phoenix / Evidently / StatsD); strategy + observability v0.3 polish. Every workspace member bumped to `0.2.0`. Git tag only — never published to PyPI (`agentforge` name was squatted; superseded by v0.2.1). |
| **v0.2.1** | 2026-05-15 | First PyPI-published release. Distribution renamed `agentforge` → `agentforge-py` (Python import name unchanged). Cross-package deps pinned to `~= 0.2.1`. Trusted Publishing workflow at `.github/workflows/release.yml`. Same shipped surface as v0.2.0. |
| **v0.3.0** | 2026-06-16 | "Application" — dogfooding-driven config + integration train. feat-026 application config (`app:` namespace + `app_as`, registered sections, `imports:` multi-file); enh-004 provider `config:` passthrough + Bedrock STS assume-role; enh-003 MCP `from_http(middleware=…)` auth seam; bug-021 uv-aware `add module`; bug-022 memory-from-config; bug-023 replay strategy reconstruction; offline configured-agent E2E net (per-PR + pre-publish wheel gate). Every workspace member bumped to `0.3.0`. (Patch trains 0.2.2–0.2.4 shipped between; see `docs/releases/`.) |
| **v0.3.1** | 2026-06-17 | Patch. bug-024 version reporting (`--version` + every `__version__` now sourced from distribution metadata, can't drift); packaged-wheel E2E gate asserts the installed artifact's version. Security: pyjwt 2.12.1 → 2.13.0 (five advisories; two touch the MCP auth seam). Dependency bumps: cryptography 48.0.1, aiohttp 3.14.1, python-multipart 0.0.31, litestar 2.22.0. Every workspace member bumped to `0.3.1`. |
| **v0.4.0** | 2026-06-23 | "Graph + governance foundation." feat-027 embedded `KuzuGraphStore` — zero-ops, file-backed, in-process graph driver (passes `run_graph_conformance`; makes the graph + GraphRAG path testable offline); new `agentforge-memory-kuzu`. enh-005 directional GraphRAG expansion (`direction: in | out | any`). enh-006 `agentforge upgrade --notes` offline drift report (from a `release_notes.json` shipped in the wheel) + `@deprecated` registry. feat-029 governance pillar 1 — identity: `Principal` widened additively (`kind` / `owner`) + new `IdentityProvider` contract + conformance in core + offline `local` driver (HMAC, URN ids) in the new `agentforge-governance` umbrella package (`governance.identity` config; ADR-0023). bug-025 `agentforge upgrade` data-loss fix (skips forked files, preserves `agentforge:custom` blocks, per-file `--dry-run` plan). feat-028 durable execution + human-in-the-loop specced (builds on the 0.5 train). Two new packages; every workspace member bumped to `0.4.0`. |
